...
- Exact 3840x2160 / 7680x4320 output, portrait transform retention, original frame timestamps, audio offsets, compressed-audio passthrough with AAC-compatible fallback, and metadata remux
- Hardware-required HEVC probes that encode three frames, finish a test movie, reopen it, and validate dimensions before 8K is exposed
- 5-second segmented 8K/long-quality jobs, atomic checkpoints, compatible resume, and segment assembly
...
HDR preservation is deliberately blocked in the current AI path because it has not yet been verified end-to-end as 10-bit. HDR sources can be explicitly headroom-tone-mapped and encoded as Rec.709 SDR; the app never silently claims that an 8-bit result preserved HDR.
