...
final class SegmentedProcessingCoordinator {
    ...
    func process(
        ...
        guard let finalURL = job.outputURL else {
            throw AppError.exportFailed("Missing segmented output destination.")
        }
        let segments = SegmentPlan.segments(duration: job.assetInfo.duration)
        guard !segments.isEmpty else {
            throw AppError.exportFailed("The source has no processable duration.")
        }
        ...
        var appliedDenoiseMethod = checkpoint.denoiseMethod
        var appliedEnhancementMethod = checkpoint.enhancementMethod
        var enhancementFallbackReason = checkpoint.enhancementFallbackReason
        var enhancementFailureCount = checkpoint.enhancementFailureCount
        ...
        for segment in segments {
               ...
               FileManager.default.fileExists(atPath: existing.path) {
                if await validateEnhancedSegment(existing, segment: segment, job: job) {
                    ...
                    completedBytes += Int64(
                        (try? existing.resourceValues(forKeys: [.fileSizeKey]).fileSize) ?? 0
                    )
            ...
            do {
                ...
                segmentInfo.estimatedSourceBytes = Int64(
                    (try? extracted.resourceValues(forKeys: [.fileSizeKey]).fileSize) ?? 0
                )
                ...
                segmentJob = try await aiPipeline.process(
                    ...
                    outputBytes: { localBytes in
                        outputBytes(segmentBaseBytes + localBytes)
                    }
                )

                appliedDenoiseMethod = segmentJob.denoiseMethod ?? appliedDenoiseMethod
                appliedEnhancementMethod = segmentJob.enhancementMethod ?? appliedEnhancementMethod
                if let reason = segmentJob.enhancementFallbackReason {
                    enhancementFallbackReason = reason
                }
                enhancementFailureCount += segmentJob.enhancementFailureCount ?? 0

                guard segmentJob.status == .completed,
                      FileManager.default.fileExists(atPath: enhanced.path) else {
                    throw AppError.exportFailed(
                        "Enhanced segment " + String(segment.index + 1) + " did not complete."
                    )
                }
                ...
                checkpoint.completedSegments = Array(Set(checkpoint.completedSegments)).sorted()
                checkpoint.completedSegmentFiles[String(segment.index)] = enhanced
                checkpoint.lastPresentationSeconds = segment.endSeconds
                checkpoint.denoiseMethod = appliedDenoiseMethod
                checkpoint.enhancementMethod = appliedEnhancementMethod
                checkpoint.enhancementFallbackReason = enhancementFallbackReason
                checkpoint.enhancementFailureCount = enhancementFailureCount
                checkpoint.updatedAt = Date()
                try await checkpointStore.save(checkpoint)

                completedFiles.append(enhanced)
                completedBytes += Int64(
                    (try? enhanced.resourceValues(forKeys: [.fileSizeKey]).fileSize) ?? 0
                )
                outputBytes(completedBytes)
            } catch {
                try? FileManager.default.removeItem(at: extracted)
        ...
        result.denoiseMethod = appliedDenoiseMethod
            ?? (job.configuration.denoise > 0 ? "Applied per segment" : "Off")
        result.enhancementMethod = appliedEnhancementMethod
        result.enhancementFallbackReason = enhancementFallbackReason
        result.enhancementFailureCount = enhancementFailureCount
    ...
    private func assemble(segmentURLs: [URL], sourceURL: URL, outputURL: URL) async throws {
        ...
        guard let videoDestination = composition.addMutableTrack(
            ...
            throw AppError.exportFailed("The segmented video assembly track could not be created.")
        }
        ...
        for url in segmentURLs {
            ...
            if firstTransform == nil {
                firstTransform = try await video.load(.preferredTransform)
            }
    ...
    private func validateEnhancedSegment(
        _ url: URL,
        ...
        do {
            ...
            guard duration.isFinite,
                  duration > 0,
                  abs(duration - segment.durationSeconds) <= frameTolerance else {
                return false
            }
            ...
            return abs(abs(display.width) - expectedWidth) < 1
                && abs(abs(display.height) - expectedHeight) < 1
        } catch {
            return false
        }
    }
    ...
    private func sourceFingerprint(job: ProcessingJob) throws -> String {
        let values = try job.sourceURL.resourceValues(
